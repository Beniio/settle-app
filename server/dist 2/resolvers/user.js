...
let UserResolver = class UserResolver {
    ...
    async login(usernameOrEmail, password, { em, req }) {
        const user = await em.findOneOrFail(User_1.User, usernameOrEmail.includes('@')
            ? {
    ...
    logout({ req, res }) {
        console.log('clear cookie' + constants_1.COOKIE_NAME);
        return new Promise((resolve) => req.session.destroy((err) => {
            res.clearCookie(constants_1.COOKIE_NAME).send();
            if (err) {
                console.log(err);
                resolve(false);
                return;
